...
let reduce_ints n m = if n > m then (n - m, m) else (n, m - n);;
let rec gcd n m = 
  if n = m then n else
    let (n, m) = reduce_ints n m in
    gcd n m;;
...
let rec multiple_upto n r = if r < 2 then false else
  if n mod r = 0 then true else
    multiple_upto n (r - 1);;
...
let is_prime n = 
  ...
  if n= 2 then true else
    let i = 2 in
    let rec prime_test n i = 
      if i > (n / 2) + 1 then true else
...
is_prime 17;;                   (* : bool = true *)

is_prime 4;;                    (* : bool = false *)
is_prime 121;;                  (* : bool = false *)

is_prime 1;;                    (* : bool = false *)
is_prime 2;;                    (* : bool = true *)
is_prime 3;;                    (* : bool = true *)
